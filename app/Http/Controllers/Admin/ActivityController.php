...
class ActivityController extends ProBaseController
{
    //
    //产品列表
    ...
    {
        $res = $this->showIndex('activity');
        return view('admin.actType.activity.list',$res);
    }
    ...
    public function store()
    {
        ...
        $res = $this->addData('activity', 'add', $input, 0, $rules, $message);
        if($res['code'] == 1){
            if($res['state'])
                return ['state'=>1, 'msg'=>'添加成功', 'url'=>url('admin/activity')];
            else
    ...
    {
        $res = $this->addView();
        return view('admin.actType.activity.add', $res);
    }
    ...
    {
        $res = $this->editView('activity', $pro_id);
        return view('admin.actType.activity.change',$res);
    }
    ...
    public function update($pro_id)
    {
        ...
        $res = $this->addData('activity', 'edit', $input, $pro_id, $rules, $message);
        if($res['code'] == 1){
            if($res['state'])
                return ['state'=>1, 'msg'=>'修改成功', 'url'=>url('admin/activity')];
            else
    ...
    {
        return $this->copyData('activity', $copy_id);
    }
    ...
    {
        return $this->del('activity', $pro_id) ? ['status' => 1, 'msg' => '删除成功！']: ['status' => 0, 'msg' => '删除失败！'];
    }
