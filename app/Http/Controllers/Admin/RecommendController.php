...
class RecommendController extends ProBaseController
{
    //
    //产品列表
    ...
    {
        $res = $this->showIndex('recommend');
        return view('admin.actType.actpush.list',$res);
    }
    ...
    public function store()
    {
        ...
        $res = $this->addData('recommend', 'add', $input, 0, $rules, $message);
        if($res['code'] == 1){
            if($res['state'])
                return ['state'=>1, 'msg'=>'添加成功', 'url'=>url('admin/recommend')];
            else
    ...
    {
        $res = $this->addView();
        return view('admin.actType.actpush.add', $res);
    }
    ...
    {
        $res = $this->editView('recommend', $pro_id);
        return view('admin.actType.actpush.change',$res);
    }
    ...
    public function update($pro_id)
    {
        ...
        $res = $this->addData('recommend', 'edit', $input, $pro_id, $rules, $message);
        if($res['code'] == 1){
            if($res['state'])
                return ['state'=>1, 'msg'=>'修改成功', 'url'=>url('admin/recommend')];
            else
    ...
    {
        return $this->copyData('recommend', $copy_id);
    }
    ...
    {
        return $this->del('recommend', $pro_id) ? ['status' => 1, 'msg' => '删除成功！']: ['status' => 0, 'msg' => '删除失败！'];
    }
