...
//                ['is_delete',0],
    ...
    public function handleOrder(Request $request)
    {
        ...
        Redis::set('Cart',Redis::get('Cart_buy'));
        $datas = $request -> except('_token');
        $uid = Auth::id();//用户id
        ...
        if(count($orderInfo)>1){
            ...
            try{
                ...
                foreach($orderInfo as $k=>$v){
                    //商品id
                    $pid = $v->proId;
                    $picName1 = session($uid.'_'.$pid.'_'.$v -> typeNumber.'_'.'picName1');
                    $picName2 = session($uid.'_'.$pid.'_'.$v -> typeNumber.'_'.'picName2');
                    //清除session
                    $request -> session() -> forget($uid.'_'.$pid.'_'.$v -> typeNumber.'_'.'picName1');
                    $request -> session() -> forget($uid.'_'.$pid.'_'.$v -> typeNumber.'_'.'picName2');
                    if(empty($picName1) && empty($picName2))
                    {
                        $field['demand1'] = '';//需求1
                        $field['demand2'] = '';//需求2
                    }else{
                        ...
                        $field['demand1'] = $picName1;//需求1
                        $field['demand2'] = $picName2;//需求2
                    }
...
//            $WechatPay->pay($orders, $amount);
            ...
            return redirect('http://youpinshanglian.com/wechat/example/jsapi.php');
            ...
            // return redirect('http://temp.yinhuishangmeng.com/wechat/example/jsapi.php');
            return redirect('http://youpinshanglian.com/wechat/example/jsapi.php?cash='.$res['order_amount']);
