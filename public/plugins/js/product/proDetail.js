...
$(".share").click(function () {
    ...
    if (isqqBrowser || isucBrowser) {
        var title ='';
        var content = '';
        title=prompt("请输入分享标题",proName);
        content=prompt("请输入分享描述","印刷我们最专业");
        // alert(age)//输出用户输入的信息
        // 用QQ和UC浏览器自带的分享功能
        var config = {
            url:shareUrl,
...
$(".listName").click(function () {
    $('.detailList a').css({"color":"#4e4e4e"});
    $(this).css({"color":"#ff3600","text-decoration":"none"});
...
$('.params li').click(function() {
    ...
    //页面中带有on的元素数量
    var hasOnNum = $(".params li.on").length;
...
$(".buyNowClick").click(function () {
    ...
    $.ajax({
        type: "POST",
        url: urlBuy,
        //data: {attrNameVal:attrNameVal,price1:price1,selfAttrObject:selfAttrObject,proId:proId,num:1},
        data: {allinfo: allinfo1},
        dataType: "json",
        headers: {
            'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
        },
...
$("#image_upload").click(function() {
    ...
    $('#image_upload').fileupload({
        ...
        messages: {
            acceptFileTypes: '，文件格式错误！核对文件格式！',
            maxNumberOfFiles: '，文件过多！',
            maxFileSize: '，单文件超过上限！（5Mb）'
        },
        headers:{
            'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
        },
